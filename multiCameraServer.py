...
if __name__ == "__main__":
    ...
    yellowCircleBuffer = np.zeros(shape=(240, 320, 3), dtype=np.uint8)
    yellowCircleStream = CameraServer.getInstance().putVideo("Yellow Circle", 320, 240)
    yellowCirclePipeline = YellowCirclePipeline()
    ...
    while True:
        ...
        stream = CameraServer.getInstance().getVideo(camera=cameras[1])
        time, yellowCircleBuffer = stream.grabFrame(yellowCircleBuffer)
        circles = yellowCirclePipeline.process(yellowCircleBuffer)
        for circle in circles:
            x, y, w, h = cv2.boundingRect(circle)
            cv2.rectangle(yellowCircleBuffer, (x,y),(x+w,y+h),(0,255,0),2)

        yellowCircleStream.putFrame(yellowCircleBuffer)




class YellowCirclePipeline:
    """
    ...
    def __init__(self):
        ...
        self.__hsv_threshold_hue = [20.0, 36.3667232597623]
        self.__hsv_threshold_saturation = [104.5863309352518, 255.0]
